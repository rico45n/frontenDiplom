...
export default function CrossesDevice(){
    ...
    const loadCross = async () => {
        const result = await axios.get("http://localhost:8080/CrossDevices/CrossDevicesAll")
        setCross(result.data)
    }

    const deleteCross = async (PoolsId) => {
        await axios.delete(`http://localhost:8080/CrossDevices/DeleteCrossDevices/${PoolsId}`)
        loadCross()
    }
    return(
        <div>
                ...
                <div className="py-4">
                    <table className="table border shadow">
                        ...
                        <tr>
                            ...
                            <th className="text-center" scope="col">Hostname начального уст-ва</th>
                            <th className="text-center" scope="col">Hostname подключаемого устройства</th>
                            <th className="text-center" scope="col">Назначеный Ip-адрес</th>
                            <th className="text-center" scope="col">Инфентарный нормер подключаемого устройства</th>
                            <th className="text-center" scope="col">Ответственный за устр-во</th>
                            <th className="text-center" scope="col">Причина изменения</th>
                            <th className="text-center" scope="col">Наименование VLAN</th>
                            <th className="text-center" scope="col">Информация о подключении</th>
                            ...
                            <td className="text-center">
                                <Link className="btn btn-success mx-2"
                                      to="/addCrossDev">Добавить в журнал подключений телекоммуникационного оборудования</Link>
                            </td>
                        ...
                        <tbody>
                        {
                            cross.map((journal, index) => (
                                <tr>
                                    <th className="text-center" scope="row" key={index}>{index + 1}</th>
                                    <td className="text-center">{journal.hostNameStart}</td>
                                    <td className="text-center">{journal.hostNameEnd}</td>
                                    <td className="text-center">{journal.ipAddressNetwork}</td>
                                    <td className="text-center">{journal.inventarNumber}</td>
                                    <td className="text-center">{journal.userOtvDev}</td>
                                    <td className="text-center">{journal.description}</td>
                                    <td className="text-center">{journal.nameVlan}</td>
                                    <td className="text-center">{journal.infoCrosses}</td>
                                    <td className="text-center">{journal.userOtvDev}</td>
                                    <td className="text-center">{journal.userOld}</td>
                                    <td className="text-center">
                                        <Link className="btn btn-outline-primary mx-2"
                                              to={`/editCrossesDevice/${journal.idCrossDevices}`}>Обновить</Link>
                                        <button className="btn btn-danger mx-2"
                                                onClick={() => deleteCross(journal.idCrossDevices)}>Удалить
                                        </button>
                                    </td>
                                </tr>
                            ))
