...
export default function VLAN() {
    ...
    const deleteVlan = async (vlansId) => {
        await axios.delete(`http://localhost:8080/Vlan/DeleteVlan/${vlansId}`)
        loadVlans()
    }
    return (
        <div>
                ...
                <div className="py-4">
                    <table className="table border shadow">
                        ...
                        <tr>
                            ...
                            <th className="text-center" scope="col">Номер VLAN</th>
                            <td className="text-center">
                                <Link className="btn btn-success mx-2"
                                      to="/addVLAN">Добавить VLAN</Link>
                            </td>
                        ...
                        <tbody>
                        {
                            vlans.map((vlan, index) => (
                                <tr>
                                    ...
                                    <td className="text-center">{vlan.vlanName}</td>
                                    <td className="text-center">{vlan.vlanNumber}</td>
                                    <td className="text-center">
                                        <Link className="btn btn-outline-primary mx-2"
                                              to={`/editVlan/${vlan.idVlan}`}>Обновить</Link>
                                        <button className="btn btn-danger mx-2"
                                                onClick={() => deleteVlan(vlan.idVlan)}>Удалить
                                        </button>
                                    </td>
                                </tr>
                            ))
